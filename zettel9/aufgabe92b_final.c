...
Über Zeilen- und Spaltenkommunikatoren werden vom Rootprozess per MPI_Gather die Werte der anderen Prozesse geholt und sortiert. Jeder Prozess bekommt 
dann die für sich relevanten Daten vom Rootprozess zugeschickt.
...
{ 		
	...
	// Zeilen sortieren
		...
		{
		// Zeilen einsammeln
			MPI_Gather(data[i], ratio, MPI_INT, recvbuf, ratio, MPI_INT, ROOT, comm_row);
		...
		// Passende Werte wieder verschicken
			MPI_Scatter(recvbuf, ratio, MPI_INT, data[i], ratio, MPI_INT, ROOT, comm_row);
		}
	...
	// Spalten sortieren
		...
		// Spalten einsammeln
			MPI_Gather(col_array, ratio, MPI_INT, recvbuf, ratio, MPI_INT, ROOT, comm_col);

			// Spalten sortieren
			qsort(recvbuf,N,sizeof(int),compare_asc);
			
		// Passende Werte wieder verschicken
			MPI_Scatter(recvbuf, ratio, MPI_INT, col_array, ratio, MPI_INT, ROOT, comm_col);

		// Spaltenarray speichern
			int k;
			for(k = 0; k < ratio; k++){
				data[k][i] = col_array[k];
			}
		}
		MPI_Barrier(MPI_COMM_WORLD);
